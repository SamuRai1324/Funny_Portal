...
function initGlobalChat() {
    ...
    form.addEventListener('submit', function(e) {
        if (chatCooldown > 0) {
            e.preventDefault();
            if (typeof showToast === 'function') showToast('Подождите ' + chatCooldown + ' сек', 'warning');
        }
    });
...
async function refreshChat() {
    ...
    try {
        ...
        var ids = new Set(
            Array.from(c.querySelectorAll('.chat-message')).map(function(m) { return m.dataset.messageId; })
        );
        msgs.forEach(function(msg) {
            if (!ids.has(String(msg.id))) {
                ...
                var av = msg.avatar
                    ? '<img src="/uploads/' + msg.avatar + '">'
                    : '<span>' + msg.username[0].toUpperCase() + '</span>';
                d.innerHTML = '<div class="chat-message-avatar">' + av + '</div>' +
                    '<div class="chat-message-content">' +
                    '<a href="/profile/' + msg.username + '" class="chat-message-author">' + msg.username + '</a>' +
                    '<div class="chat-message-text">' + escapeHtmlChat(msg.content) + '</div></div>';
                c.appendChild(d);
            }
        });
...
function updateCooldownDisplay() {
    ...
    if (d) {
        d.style.display = chatCooldown > 0 ? 'block' : 'none';
        d.textContent = 'Подождите: ' + chatCooldown + ' сек';
    }
}
